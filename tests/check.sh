...
grep -Fq 'v0.3.1' "$ROOT/nvim/vibe.lua"
...
grep -Fq 'if-shell -F "#{pane_active}" "send-keys -M" "select-pane -t ="' "$ROOT/tmux.conf"
grep -Fq 'mouse-pane' "$ROOT/tmux.conf"
grep -Fq '@vibe_tab_helper' "$ROOT/tmux.conf"
grep -Fq '#[align=left]' "$ROOT/tmux.conf"
test ! -e "$ROOT/bin/vibe-favorite"
test ! -e "$ROOT/yazi/plugins/vibe-favorites.yazi/main.lua"
! grep -Fiq 'favorite' "$ROOT/README.md"
! grep -Fq 'VIBE_FAVORITES' "$ROOT/bin/vibe" "$ROOT/yazi/init.lua" "$ROOT/yazi/keymap.toml"
! grep -Fq 'vibe-favorites' "$ROOT/yazi/init.lua" "$ROOT/yazi/keymap.toml"
